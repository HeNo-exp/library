...
--[[
    ...
    @buildDate "2026-07-22T16:08:06.392850Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			-- AutomaticSize 루프 박멸: 오버레이 물리 크기는 부모 Row 크기와 100% 동일하게 유지
			local parentSize = row.AbsoluteSize
			overlay.Position = UDim2.fromScale(0, 0)
			overlay.Size = UDim2.new(0, parentSize.X, 0, parentSize.Y)

			-- 비주얼 요소(BlurBackground)만 삐져나가게 해서 구분선 영역을 덮음
			...
			if blurBg then
				...
				if not isFirst then
					-- 위쪽이 잠겨 있으면 위로 1픽셀 늘려 틈을 메움
					offsetPosY = -1
					offsetSizeY = offsetSizeY + 1
				end
				if not isLast then
					-- 아래쪽이 잠겨 있으면 아래로 2픽셀 늘려 구분선(Divider)을 덮어버림
					offsetSizeY = offsetSizeY + 2
				end

				blurBg.Position = UDim2.new(0, 0, 0, offsetPosY)
				blurBg.Size = UDim2.new(1, 0, 1, offsetSizeY)
			end

			local lockIcon = overlay:FindFirstChild("LockIcon")
			local lockText = overlay:FindFirstChild("LockText")

			-- 중앙 Row에만 자물쇠 아이콘 및 텍스트 표시
			local showDetails = (idx == centerIndex)
			if lockIcon then
				lockIcon.Visible = showDetails
			end
			if lockText then
				lockText.Visible = showDetails
			end
		end
	end
end
...
local function createLockOverlay(parentInstance: Instance)
	...
	-- 초기 크기를 AbsoluteSize 기반으로 고정 (AutomaticSize로 팽창되는 버그 방지)
	local initSize = parentInstance.AbsoluteSize
	overlay.Size = UDim2.fromOffset(initSize.X, initSize.Y)
	overlay.Position = UDim2.fromScale(0, 0)
	overlay.BackgroundColor3 = Color3.fromRGB(10, 10, 10) -- 조금 더 어두운 다크 그레이로 변경
	overlay.BackgroundTransparency = 1
	overlay.BorderSizePixel = 0
	overlay.ZIndex = 100
	overlay.Active = true
	overlay.ClipsDescendants = false -- 비주얼 합침(Blur Bg 연장)을 위해 하위 클리핑 끔
	...
	-- 터치 및 클릭 이벤트를 완벽하게 차단하기 위한 투명 Blocker Button 추가
	...
	blocker.Text = ""
	blocker.Active = true
	blocker.ZIndex = 98 -- 다른 장식들 아래에 배치
	...
	lockIcon.Position = UDim2.fromScale(0.5, 0.38)
	...
	lockText.Position = UDim2.fromScale(0.5, 0.72)
	...
	return function(self, ...)
		...
		if typeof(result) == "table" and final ~= nil then
			result.Lock = function(elementSelf, isLocked: boolean)
				...
				local overlay = targetInstance:FindFirstChild("LockOverlay")
				...
				if isLocked then
					...
					-- 페이드인 애니메이션 (블러 강도를 대폭 높임)
					tweenService:Create(overlay, tweenInfo, { BackgroundTransparency = 0.65 }):Play()
					if blurBg then
						-- 블러 이미지의 투명도를 0.05로 주어 흐릿함을 아주 강하게 강조
					...
					if overlay then
						...
						local t1 = tweenService:Create(overlay, tweenInfo, { BackgroundTransparency = 1 })
						...
						task.delay(0.3, function()
							local currentOverlay = targetInstance:FindFirstChild("LockOverlay")
