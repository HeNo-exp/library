...
--[[
    ...
    @buildDate "2026-07-23T08:15:19.083425Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			-- 비주얼 크기를 부모 영역 스케일 기반(100% 매칭)으로 고정하여 팽창 버그 영구 봉쇄!
			...
			if blurBg then
				...
				blurBg.Size = UDim2.new(1, 12, 1, 0) -- scale Y = 1, offset Y = 0 고정
			end
			if blocker then
				blocker.Visible = true
				blocker.Position = UDim2.new(0, -6, 0, 0)
				blocker.Size = UDim2.new(1, 12, 1, 0) -- scale Y = 1, offset Y = 0 고정
			end

			-- 병합 시 코너 반경 제어 (2개 이상 결합되면 모서리를 직각으로 깎아 틈새 없는 일체형 블랙 창 연출)
			local parentCorner = row:FindFirstChildOfClass("UICorner")
			local originalRadius = parentCorner and parentCorner.CornerRadius or UDim.new(0, 8)
			local targetRadius = (size > 1) and UDim.new(0, 0) or originalRadius

			for _, corner in ipairs(overlay:GetChildren()) do
				if corner:IsA("UICorner") then
					corner.CornerRadius = targetRadius
				end
			end
			if blocker then
				local blockerCorner = blocker:FindFirstChildOfClass("UICorner")
				if blockerCorner then
					blockerCorner.CornerRadius = targetRadius
				end
			end
			if blurBg then
				local blurCorner = blurBg:FindFirstChildOfClass("UICorner")
				if blurCorner then
					blurCorner.CornerRadius = targetRadius
				end
			end
			...
			if size > 1 then
				...
				if lockIcon then
					...
					lockIcon.Position = UDim2.new(0.5, 0, 0.5, 0) -- 해당 Row의 정중앙 Y축 고정
					if showIcon then
				...
				if lockTitle then
					...
					if isFirst then
						...
						lockTitle.Position = UDim2.new(0, 14, 0, 10)
						lockTitle.Size = UDim2.new(0.6, 0, 0, 20)
				...
				if lockSubtitle then
					...
					if isFirst and hasSub then
						...
						lockSubtitle.Position = UDim2.new(0, 14, 0, 32) -- 타이틀 22px 아래 배치
						lockSubtitle.Size = UDim2.new(0.6, 0, 0, 14)
				...
				if lockIcon then
					...
					lockIcon.Position = UDim2.new(0.5, 0, 0.28, 0)
					tweenService:Create(lockIcon, tweenInfo, { ImageTransparency = 0.0 }):Play()
				end
				...
				if lockTitle then
					...
					lockTitle.Size = UDim2.new(0.8, 0, 0.24, 0)
				...
				if lockSubtitle then
					...
					if hasSub then
						...
						lockSubtitle.Size = UDim2.new(0.8, 0, 0.18, 0)
