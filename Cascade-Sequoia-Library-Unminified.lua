...
--[[
    ...
    @buildDate "2026-07-23T08:03:46.850733Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
		...
		for idx, row in ipairs(group) do
			...
			if size > 1 then
				if isFirst then
					...
					-- 큼직한 커스텀 소제목 제목 하단 배치
					if lockSubtitle then
						local hasSub = (subtitleText ~= "")
						lockSubtitle.Visible = hasSub
						if hasSub then
							lockSubtitle.AnchorPoint = Vector2.new(0, 0)
							lockSubtitle.Position = UDim2.new(0, 14, 0, 42) -- 타이틀 28px 아래 배치
							lockSubtitle.Size = UDim2.new(0.6, 0, 0, 16)
