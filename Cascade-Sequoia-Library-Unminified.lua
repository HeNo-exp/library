...
--[[
    ...
    @buildDate "2026-07-23T07:00:00.505943Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function createLockOverlay(parentInstance: Instance)
	...
	-- 터치 및 클릭 이벤트를 차단하고, 반투명 하얀색 배경 역할을 수행할 Blocker Button
	local blocker = Instance.new("TextButton")
	blocker.Name = "Blocker"
	blocker.Position = UDim2.fromScale(0, 0)
	blocker.Size = UDim2.new(1, 0, 0, parentInstance.AbsoluteSize.Y)
	blocker.BackgroundColor3 = Color3.fromRGB(255, 255, 255) -- 하얀색 아크릴 필터로 변경
	blocker.BackgroundTransparency = 1 -- 트윈으로 제어 예정 (목표치 0.85)
	...
	blurBg.ImageColor3 = Color3.fromRGB(255, 255, 255) -- 하얀색 블러 이미지
	...
	-- 가독성을 높이기 위해 은은한 블랙 윤곽선(Stroke) 추가
	lockText.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
	lockText.TextStrokeTransparency = 1 -- 트윈 또는 락 활성화 시 0.8로 지정
		...
		if typeof(result) == "table" and final ~= nil then
			result.Lock = function(elementSelf, isLocked: boolean)
				...
				if isLocked then
					...
					-- 페이드인 애니메이션 (하얀색 아크릴 및 흰색 블러 강도를 대폭 높임)
					if blocker then
						tweenService:Create(blocker, tweenInfo, { BackgroundTransparency = 0.85 }):Play()
					end
					if blurBg then
						tweenService:Create(blurBg, tweenInfo, { ImageTransparency = 0.05 }):Play()
					end
					...
					if lockText and lockText.Visible then
						tweenService:Create(lockText, tweenInfo, { TextTransparency = 0.0, TextStrokeTransparency = 0.8 }):Play() -- 선명하게 표시 + 그림자 적용
					else
						if lockText then 
							lockText.TextTransparency = 1 
					...
					if overlay then
						...
						local t4 = lockText and tweenService:Create(lockText, tweenInfo, { TextTransparency = 1, TextStrokeTransparency = 1 })
