...
--[[
    ...
    @buildDate "2026-07-23T07:14:21.344633Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function createLockOverlay(parentInstance: Instance)
	...
	-- 흰색 텍스트가 돋보이도록 은은한 블랙 Stroke 적용
			...
			result.Lock = function(elementSelf, isLocked: boolean)
				...
				if isLocked then
					...
					if lockText and lockText.Visible then
						tweenService:Create(lockText, tweenInfo, { TextTransparency = 0.0, TextStrokeTransparency = 0.8 }):Play() -- 선명하게 표시 + 그림자
					else
						if lockText then 
							lockText.TextTransparency = 1 
							lockText.TextStrokeTransparency = 1
