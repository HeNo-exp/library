...
--[[
    ...
    @buildDate "2026-07-23T06:55:31.904744Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function createLockOverlay(parentInstance: Instance)
	...
	-- 터치 및 클릭 이벤트를 차단하고, 반투명 어두운 배경 역할을 수행할 Blocker Button
	local blocker = Instance.new("TextButton")
	blocker.Name = "Blocker"
	blocker.Position = UDim2.fromScale(0, 0)
	blocker.Size = UDim2.new(1, 0, 0, parentInstance.AbsoluteSize.Y)
	blocker.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
	blocker.BackgroundTransparency = 1 -- 트윈으로 제어 예정
	...
	lockIcon.ImageColor3 = Color3.fromRGB(240, 240, 240)
		...
		if typeof(result) == "table" and final ~= nil then
			result.Lock = function(elementSelf, isLocked: boolean)
				...
				if isLocked then
					...
					-- 페이드인 애니메이션 (블러 및 다크 필터 강도를 대폭 높임)
					if blocker then
						tweenService:Create(blocker, tweenInfo, { BackgroundTransparency = 0.65 }):Play()
					end
					if blurBg then
						tweenService:Create(blurBg, tweenInfo, { ImageTransparency = 0.05 }):Play()
					end
					if lockIcon and lockIcon.Visible then
						tweenService:Create(lockIcon, tweenInfo, { ImageTransparency = 0.15 }):Play()
					else
						if lockIcon then lockIcon.ImageTransparency = 1 end
					end
					if lockText and lockText.Visible then
						tweenService:Create(lockText, tweenInfo, { TextTransparency = 0.15 }):Play()
					else
						if lockText then lockText.TextTransparency = 1 end
					end
				else
					if overlay then
