...
--[[
    ...
    @buildDate "2026-06-19T20:31:55.548257700+00:00"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
return function(self, properties: any)
	...
	local function openPopover()
		...
		local ok, err = xpcall(function()
			...
			popoverFrame = create("Frame")({
				...
				BorderSizePixel = 0,
				Parent = screenGui,
				ZIndex = 1000, -- Draw above other elements
			...
			local middleSplit = popoverFrame:FindFirstChild("MiddleSplit") :: Frame
			local visualSpec = middleSplit:FindFirstChild("VisualSpec") :: ImageLabel
			local inputsFrame = middleSplit:FindFirstChild("InputsFrame") :: Frame

			-- Inputs
			local hexInput = createInputField("Hex", colorToHex(tempColor), 1, inputsFrame)
			local rInput = createInputField("Red", math.floor(tempColor.R * 255), 2, inputsFrame)
			local gInput = createInputField("Green", math.floor(tempColor.G * 255), 3, inputsFrame)
			local bInput = createInputField("Blue", math.floor(tempColor.B * 255), 4, inputsFrame)
			local aInput = createInputField("Alpha", math.floor((1 - tempTransparency) * 100), 5, inputsFrame)
			...
			local track = sliderFrame:FindFirstChild("Track") :: ImageButton
			local thumb = track:FindFirstChild("Thumb") :: Frame
			...
			local previewsFrame = create("Frame")({
				...
				create("Frame")({
					...
					create("UIStroke")({ Color = Color3.fromRGB(255, 255, 255), Transparency = 0.8 }),
				}),
			...
			local newColorBar = previewsFrame:FindFirstChild("NewColor") :: Frame
			...
			local actionButtonsFrame = create("Frame")({
				Name = "ActionsFrame",
			...
			local confirmBtn = actionButtonsFrame:FindFirstChild("Confirm") :: TextButton
			local cancelBtn = actionButtonsFrame:FindFirstChild("Cancel") :: TextButton

			local function closePopover()
				if popoverFrame then
					popoverFrame:Destroy()
					popoverFrame = nil
				end
			end
			...
			popoverFrame.Position = UDim2.fromOffset(posX, posY)
			print("[ColorPicker] Popover frame positioned at:", posX, posY, "and parented to:", popoverFrame.Parent:GetFullName())

			-- Listen for click outside
			task.defer(function()
				local conn
				conn = userInputService.InputBegan:Connect(function(input)
					if not popoverFrame then
						conn:Disconnect()
						return
					end
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
						...
						local insidePopover = mouse.X >= popoverFrame.AbsolutePosition.X 
							and mouse.X <= popoverFrame.AbsolutePosition.X + popoverFrame.AbsoluteSize.X
							and adjustedY >= popoverFrame.AbsolutePosition.Y
							and adjustedY <= popoverFrame.AbsolutePosition.Y + popoverFrame.AbsoluteSize.Y
	...
	print("[ColorPicker] Connecting MouseButton1Click for openPopover...")
	structures.ColorButton.MouseButton1Click:Connect(openPopover)
