...
--[[
    ...
    @buildDate "2026-06-20T03:48:48.695728400+00:00"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local creator = __DIST.d()
	...
	object = binder.Wrap(properties, {
		...
		Selected = function(selected: boolean)
			...
			tweenService:Create(structures.Body.__instance, tweenInfo, { BackgroundTransparency = targetTransparency }):Play()
			tweenService:Create(structures.Title.__instance, tweenInfo, { TextColor3 = targetTitleColor, TextTransparency = targetTitleTransparency }):Play()
			tweenService:Create(structures.Symbol.__instance, tweenInfo, { ImageColor3 = targetSymbolColor, ImageTransparency = targetSymbolTransparency }):Play()

			if selected then
				structures.Page.Parent = window and window.__container or nil
				structures.Page.Position = UDim2.new(0, 0, 0, 15)
				tweenService:Create(structures.Page.__instance, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
	...
	object.Navigate = function(self, targetPage: Page__DARKLUA_TYPE_N)
		...
		if object.Selected and window and window.__container then
			...
			tweenService:Create(newPageStructure.__instance, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
				Position = UDim2.new(0, 0, 0, 0),
			}):Play()
		end
