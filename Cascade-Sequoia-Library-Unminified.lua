...
--[[
    ...
    @buildDate "2026-07-21T16:18:31.804538Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local creator = __DIST.d()
	...
	local function drawChart()
		-- Preserve UIScale by only destroying non-UIScale children
		for _, child in ipairs(getRaw(chartFrame):GetChildren()) do
		...
		-- Optional smooth animation if Animated / LiveAnimation is true
		local isAnimated = properties.Animated ~= false and properties.LiveAnimation ~= false
		local rawChart = getRaw(chartFrame)
		local rawScale = getRaw(uiScale)
		if isAnimated then
			rawScale.Scale = 0.88
			rawChart.GroupTransparency = 0.2
			tweenService:Create(rawScale, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Scale = 1,
			}):Play()
			tweenService:Create(rawChart, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				GroupTransparency = 0,
			}):Play()
		else
			rawScale.Scale = 1
			rawChart.GroupTransparency = 0
		end
