...
--[[
    ...
    @buildDate "2026-07-23T07:34:32.835956Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			if size > 1 then
				...
				-- 2개 이상 결합 시 첫 번째 Row의 우측 상단에 커스텀 제목/소제목 배치
				if lockTitle then
					lockTitle.Visible = isFirst
					if isFirst then
						lockTitle.AnchorPoint = Vector2.new(1, 0)
						lockTitle.Position = UDim2.new(1, -12, 0, 10)
						lockTitle.TextXAlignment = Enum.TextXAlignment.Right
				...
				if lockSubtitle then
					...
					if isFirst and hasSub then
						lockSubtitle.AnchorPoint = Vector2.new(1, 0)
						lockSubtitle.Position = UDim2.new(1, -12, 0, 24) -- 타이틀 아래 배치
						lockSubtitle.TextXAlignment = Enum.TextXAlignment.Right
						lockSubtitle.TextSize = 9
				...
				if lockIcon then
					...
					lockIcon.Position = UDim2.new(0.5, 0, 0, parentSize.Y * 0.32)
					tweenService:Create(lockIcon, tweenInfo, { ImageTransparency = 0.0 }):Play()
				end
				...
				if lockTitle then
					...
					lockTitle.Position = UDim2.new(0.5, 0, 0, parentSize.Y * (hasSub and 0.58 or 0.68))
				...
				if lockSubtitle then
					...
					if hasSub then
						...
						lockSubtitle.Position = UDim2.new(0.5, 0, 0, parentSize.Y * 0.78)
...
local function createLockOverlay(parentInstance: Instance)
	...
	lockTitle.Size = UDim2.new(1, 0, 0, 12)
	...
	lockSubtitle.Size = UDim2.new(1, 0, 0, 10)
