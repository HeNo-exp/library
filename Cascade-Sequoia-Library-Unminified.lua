...
--[[
    ...
    @buildDate "2026-07-23T06:40:39.616810Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			-- 물리적인 팽창 차단: 오버레이 크기를 완벽히 scale 1로 고정하여 AutomaticSize 영향 제거
			...
			-- BlurBackground 크기도 팽창 없이 Scale 1로 고정
			...
			if size > 1 then
				...
				if lockIcon then
					...
					lockIcon.Position = UDim2.fromScale(0.5, 0.5) -- 중앙 정렬
				end
...
local function createLockOverlay(parentInstance: Instance)
	...
	-- AutomaticSize 오버플로우 방지: 완벽한 Scale 1 지정
	overlay.Size = UDim2.fromScale(1, 1)
	overlay.Position = UDim2.fromScale(0, 0)
	...
	overlay.ZIndex = 100
	overlay.Active = true
	overlay.ClipsDescendants = false -- 비주얼 합침을 위해 하위 클리핑 끔
	...
	-- Row 내의 LayoutIgnore 폴더(정렬/AutomaticSize 무시 구역)에 오버레이를 배치하여 팽창 원천 제거
