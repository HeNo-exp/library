...
--[[
    ...
    @buildDate "2026-06-22T16:27:51.544172Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function parseDirection(dir)
	if typeof(dir) == "number" then
		return dir
	end
	if typeof(dir) == "string" then
		local d = string.lower(dir)
		if d == "l2r" or d == "lefttoright" or d == "horizontal" or d == "right" or d == "옆에서오른쪽" or d == "왼쪽에서오른쪽" or d == "왼쪽오른쪽" or d == "옆에서우측" then
			return 0
		elseif d == "t2b" or d == "toptobottom" or d == "vertical" or d == "down" or d == "위에서밑" or d == "위에서아래" or d == "위아래" then
			return 90
		elseif d == "r2l" or d == "righttoleft" or d == "left" or d == "오른쪽에서옆" or d == "오른쪽에서왼쪽" or d == "오른쪽왼쪽" then
			return 180
		elseif d == "b2t" or d == "bottomtotop" or d == "up" or d == "밑에서위" or d == "아래에서위" or d == "아래위" then
			return 270
		elseif d == "tl2br" or d == "toplefttobottomright" or d == "대각선1" or d == "주대각선" then
			return 45
		elseif d == "tr2bl" or d == "toprighttobottomleft" then
			return 135
		elseif d == "bl2tr" or d == "bottomlefttotopright" then
			return 315
		elseif d == "br2tl" or d == "bottomrighttotopleft" then
			return 225
		end
	end
	return 0
...
local function parseColorSequence(colors)
	...
	if typeof(colors) == "table" then
		...
		if typeof(first) == "Color3" or typeof(first) == "string" then
			local keypoints = {}
			local n = #colors
			if n == 1 then
				local col = typeof(first) == "string" and Color3.fromHex(first) or first
				return ColorSequence.new(col)
			end
			for i, c in ipairs(colors) do
				local col = typeof(c) == "string" and Color3.fromHex(c) or c
				local t = (i - 1) / (n - 1)
				table.insert(keypoints, ColorSequenceKeypoint.new(t, col))
			end
			return ColorSequence.new(keypoints)
		elseif typeof(first) == "table" then
			local sorted = table.clone(colors)
			table.sort(sorted, function(a, b)
				local timeA = a.Time or a[1] or 0
				local timeB = b.Time or b[1] or 0
				return timeA < timeB
			end)

			local keypoints = {}
			for _, item in ipairs(sorted) do
				local t = item.Time or item[1] or 0
				local col = item.Color or item[2]
				if typeof(col) == "string" then
					col = Color3.fromHex(col)
				end
				table.insert(keypoints, ColorSequenceKeypoint.new(t, col))
			end
...
local function parseNumberSequence(transparency)
	...
	if typeof(transparency) == "table" then
		...
		local first = transparency[1]
		if typeof(first) == "number" then
			local keypoints = {}
			local n = #transparency
			if n == 1 then
				return NumberSequence.new(first)
			end
			for i, val in ipairs(transparency) do
				local t = (i - 1) / (n - 1)
				table.insert(keypoints, NumberSequenceKeypoint.new(t, val))
			end
			return NumberSequence.new(keypoints)
		elseif typeof(first) == "table" then
			local sorted = table.clone(transparency)
			table.sort(sorted, function(a, b)
				local timeA = a.Time or a[1] or 0
				local timeB = b.Time or b[1] or 0
				return timeA < timeB
			...
			for _, item in ipairs(sorted) do
				local t = item.Time or item[1] or 0
				local val = item.Value or item[2] or 0
				local env = item.Envelope or item[3] or 0
				table.insert(keypoints, NumberSequenceKeypoint.new(t, val, env))
			end
			...
			if keypoints[#keypoints].Time < 1 then
				table.insert(keypoints, NumberSequenceKeypoint.new(1, keypoints[#keypoints].Value, keypoints[#keypoints].Envelope))
			end
...
function creator.ApplyGradient(instance: Instance, properties: any)
	...
	if properties.Direction ~= nil then
		gradient.Rotation = parseDirection(properties.Direction)
	end
	if properties.Colors ~= nil then
		gradient.Color = parseColorSequence(properties.Colors)
	end
	if properties.Transparency ~= nil then
		gradient.Transparency = parseNumberSequence(properties.Transparency)
	end
	if properties.Offset ~= nil then
		gradient.Offset = properties.Offset
	end
	if properties.Enabled ~= nil then
		gradient.Enabled = properties.Enabled
	end

	return gradient
end

function creator.CreateGradient(properties: any)
	properties = properties or {}
	...
	if properties.Parent ~= nil then
		gradient.Parent = properties.Parent
	end
	...
	return function(properties)
		...
		for key, value in pairs(properties) do
			...
			if key == "Gradient" then
				if type(value) == "table" and value.Connect and value.Value ~= nil then
					creator.ApplyGradient(instance, value.Value)
					value:Connect(function(newValue)
						task.defer(pcall, creator.ApplyGradient, instance, newValue)
					end)
				else
					creator.ApplyGradient(instance, value)
				end
				continue
			end
...
local cascade = {
	...
	ApplyGradient = creator.ApplyGradient,
	CreateGradient = creator.CreateGradient,
}
