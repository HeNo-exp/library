...
--[[
    ...
    @buildDate "2026-07-23T07:31:38.458294Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			if size > 1 then
				...
				-- 2개 이상 결합 시 첫 번째 Row의 좌측 상단에 커스텀 제목/소제목 배치
				if lockTitle then
					lockTitle.Visible = isFirst
					if isFirst then
						lockTitle.AnchorPoint = Vector2.new(0, 0)
						lockTitle.Position = UDim2.new(0, 12, 0, 10)
				...
				if lockSubtitle then
					...
					if isFirst and hasSub then
						...
						lockSubtitle.Position = UDim2.new(0, 12, 0, 24) -- 타이틀 아래 배치
