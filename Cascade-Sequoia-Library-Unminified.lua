...
--[[
    ...
    @buildDate "2026-06-20T03:43:28.712703800+00:00"
    @buildCfg "Task"
    @buildVers "0.0.0"
	...
	local function openPopover()
		...
		local ok, err = xpcall(function()
			...
			local function updateWheel(input)
				...
				local inset = guiService:GetGuiInset()
				local screenGui = structures.Body:FindFirstAncestorOfClass("ScreenGui")
				local ignoreInset = (screenGui and screenGui.IgnoreGuiInset == true)
				local adjustedY = ignoreInset and mouse.Y or (mouse.Y - inset.Y)
				
				local dx = mouse.X - centerX
				local dy = adjustedY - centerY
			...
			print("[ColorPicker] Popover frame positioned at:", posX, posY, "and parented to:", panel.Parent:GetFullName())
			...
			task.defer(function()
				...
				conn = userInputService.InputBegan:Connect(function(input)
					...
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
						...
						local insidePopover = mouse.X >= panel.AbsolutePosition.X 
							and mouse.X <= panel.AbsolutePosition.X + panel.AbsoluteSize.X
							and adjustedY >= panel.AbsolutePosition.Y
							and adjustedY <= panel.AbsolutePosition.Y + panel.AbsoluteSize.Y

						local insideButton = mouse.X >= structures.ColorButton.AbsolutePosition.X
							and mouse.X <= structures.ColorButton.AbsolutePosition.X + structures.ColorButton.AbsoluteSize.X
							and adjustedY >= structures.ColorButton.AbsolutePosition.Y
							and adjustedY <= structures.ColorButton.AbsolutePosition.Y + structures.ColorButton.AbsoluteSize.Y
