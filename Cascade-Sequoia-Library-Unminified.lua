...
--[[
    ...
    @buildDate "2026-07-23T07:28:23.648011Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			-- 레이아웃 정렬 최적화 (2개 이상 병합 시 자물쇠는 중앙 Row 정중앙 배치, 자체 텍스트는 숨김)
			if size > 1 then
				...
				-- 2개 이상 결합 시 락 안내 텍스트("Locked")는 출력하지 않음 (첫 Row의 원래 제목 소제목이 노출되므로)
				if lockText then
				...
				if lockText then
					...
					tweenService:Create(lockText, tweenInfo, { TextTransparency = 0.0, TextStrokeTransparency = 0.8 }):Play()
				end
			end
		end
	end
