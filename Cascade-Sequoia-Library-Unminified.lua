...
--[[
    ...
    @buildDate "2026-06-21T01:14:16.704740Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
		...
		Selected = function(selected: boolean)
			...
			if selected and tabAnim == "move" and window and window.Structures.TabMoveIndicator then
				...
				local function isClipped(body)
					...
					while parent and parent.Name ~= "SidebarList" do
						if parent:IsA("GuiObject") and parent.ClipsDescendants then
							if parent.AbsoluteSize.Y <= 25 then
								return true
							end
						end
						parent = parent.Parent
				...
				local function updatePos(instant)
					...
					if not clipped then
						if instant or isFirstUpdate then
				...
				if object._updateThread then
					task.cancel(object._updateThread)
					object._updateThread = nil
				end

				object._updateThread = task.spawn(function()
					while object.Selected and window and window.TabAnimation == "move" and tabBody:IsDescendantOf(game) do
						updatePos(false)
						task.wait(0.03)
					end
				end)
