...
--[[
    ...
    @buildDate "2026-06-20T02:23:45.195530600+00:00"
    @buildCfg "Task"
    @buildVers "0.0.0"
	...
	local function openPopover()
		...
		local ok, err = xpcall(function()
			...
			local function updateSelectorDotFromColor(color)
				...
				local angle = math.pi / 2 - hue * 2 * math.pi
			...
			local function updateWheel(input)
				...
				local dy = centerY - adjustedY -- Y-axis flip to match standard math coordinate system
				local distance = math.sqrt(dx^2 + dy^2)
				local radius = wheelAbsSize.X / 2
				
				local angle = math.atan2(dy, dx)
				local hue = ((math.pi / 2 - angle) / (2 * math.pi)) % 1
				...
				-- Update Selector Dot Position (Roblox screen coordinate system)
				local clampedDist = math.clamp(distance, 0, radius)
				local rad = (clampedDist / radius) * 0.5
				local screenAngle = math.pi / 2 - hue * 2 * math.pi
