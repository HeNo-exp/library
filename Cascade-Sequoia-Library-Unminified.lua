...
--[[
    ...
    @buildDate "2026-07-23T08:30:19.585486Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			-- 2개 이상 결합 시, 첫 번째 Row에서만 전체 높이를 덮는 하나의 거대한 아크릴 글래스를 생성하고 나머지는 숨김!
			if size > 1 then
				if isFirst then
					if blurBg then
						blurBg.Visible = true
						blurBg.Position = UDim2.new(0, -6, 0, 0)
						blurBg.Size = UDim2.new(1, 12, 0, totalHeight) -- 전체 높이 합산 적용
					end
					if blocker then
						blocker.Visible = true
						blocker.Position = UDim2.new(0, -6, 0, 0)
						blocker.Size = UDim2.new(1, 12, 0, totalHeight) -- 전체 높이 합산 적용
					end
				else
					-- 아래쪽 잠긴 Row들의 아크릴 레이어는 가려서 중첩 및 테두리 깨짐 방지!
					if blurBg then blurBg.Visible = false end
					if blocker then blocker.Visible = false end
				end
			else
				-- 단독 락일 때는 기본 Row 크기로 작동
				if blurBg then
					blurBg.Visible = true
					blurBg.Position = UDim2.new(0, -6, 0, 0)
