...
--[[
    ...
    @buildDate "2026-06-20T03:18:02.172930+00:00"
    @buildCfg "Task"
    @buildVers "0.0.0"
	...
	object = binder.Wrap(properties, {
		...
		Selected = function(selected: boolean)
			if selected then
				...
				-- Indicator 이동 로직 (Lerp & RenderStepped 적용)
				...
				if indicator and sidebarList then
					...
					-- 인디케이터가 처음에 0,0 크기이면 첫 프레임에 즉시 텔레포트
					local isFirstFrame = (indicator.Size.X.Offset == 0 and indicator.Size.Y.Offset == 0)

					lerpConn = services.RunService.RenderStepped:Connect(function(dt)
						if not tabBtn.Parent or not indicator.Parent then
							if lerpConn then
								lerpConn:Disconnect()
								lerpConn = nil
							end
						...
						if isFirstFrame then
							...
							-- dt 보정된 lerp alpha (60fps 기준 약 0.25의 부드러운 반응속도 구현)
							local alpha = 1 - math.exp(-20 * dt)
							indicator.Position = indicator.Position:lerp(targetPos, alpha)
							indicator.Size = indicator.Size:lerp(targetSize, alpha)
						end
					end)
				end
			else
