...
--[[
    ...
    @buildDate "2026-06-20T01:22:02.482712700+00:00"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
return function(self, properties: any)
	...
	local function openPopover()
		...
		local ok, err = xpcall(function()
			...
			colorWheel.Image = "rbxassetid://2849458409" -- High quality working HSV color wheel from MacLib
			...
			local function updateSelectorDotFromColor(color)
				...
				local angle = hue * 2 * math.pi - math.pi
			...
			local h_init, s_init, v_init = Color3.toHSV(tempColor)
			...
			local function updatePopoverVisuals()
				local h, s, v = Color3.toHSV(tempColor)
			...
			local function onColorOrTransparencyUpdatedFromTextInput()
				local _, _, v = Color3.toHSV(tempColor)
			...
			local function updateWheel(input)
				...
				local _, _, value = Color3.toHSV(tempColor)
			...
			local function updateBrightness(input)
				...
				local h, s, _ = Color3.toHSV(tempColor)
