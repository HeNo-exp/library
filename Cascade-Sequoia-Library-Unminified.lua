...
--[[
    ...
    @buildDate "2026-07-23T08:22:03.753919Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	-- UIListLayout 간격(Padding) 오프셋 추출
	...
	for _, group in ipairs(groups) do
		...
		-- 병합 락 전체의 총 합산 높이 계산 (개별 Row 높이의 합 + 사이의 간격)
		local totalHeight = 0
		for _, row in ipairs(group) do
			totalHeight = totalHeight + row.AbsoluteSize.Y
		end
		if size > 1 then
			totalHeight = totalHeight + (size - 1) * paddingOffset
		end
		...
		for idx, row in ipairs(group) do
			...
			-- 병합 락일 때는 첫 번째 Row의 Blocker/BlurBg만 전체 높이로 늘려서 통째로 그리고,
			-- 나머지 아래쪽 Row들의 필터는 완전히 투명하게 꺼두어 분절 겹침 방지!
			if size > 1 then
				if isFirst then
					if blurBg then
						blurBg.Position = UDim2.new(0, -6, 0, 0)
						blurBg.Size = UDim2.new(1, 12, 0, totalHeight)
						tweenService:Create(blurBg, tweenInfo, { ImageTransparency = 0.2 }):Play()
					end
					if blocker then
			...
			-- 레이아웃 정렬 최적화 (2개 이상 병합 시 자물쇠는 중앙 Row 정중앙 배치, 제목 소제목은 첫 Row 좌측 상단 배치)
			if size > 1 then
				local showIcon = (idx == centerIndex)
				if lockIcon then
					lockIcon.Visible = showIcon
					lockIcon.Position = UDim2.new(0.5, 0, 0, parentSize.Y / 2) -- 부모 높이의 중앙
					if showIcon then
				...
				-- 2개 이상 결합 시 첫 번째 Row의 좌측 상단에 큼직한 커스텀 제목/소제목 배치
				if lockTitle then
					lockTitle.Visible = isFirst
					if isFirst then
						lockTitle.AnchorPoint = Vector2.new(0, 0)
						lockTitle.Position = UDim2.new(0, 14, 0, parentSize.Y * 0.15)
						lockTitle.Size = UDim2.new(0.6, 0, 0, parentSize.Y * 0.38)
				...
				if lockSubtitle then
					...
					lockSubtitle.Visible = isFirst and hasSub
					if isFirst and hasSub then
						lockSubtitle.AnchorPoint = Vector2.new(0, 0)
						lockSubtitle.Position = UDim2.new(0, 14, 0, parentSize.Y * 0.52)
						lockSubtitle.Size = UDim2.new(0.6, 0, 0, parentSize.Y * 0.3)
				...
				if lockTitle then
					...
					lockTitle.Position = UDim2.new(0.5, 0, 0, parentSize.Y * (hasSub and 0.55 or 0.65))
					lockTitle.Size = UDim2.new(0.8, 0, 0, parentSize.Y * 0.24)
				...
				if lockSubtitle then
					...
					if hasSub then
						...
						lockSubtitle.Position = UDim2.new(0.5, 0, 0, parentSize.Y * 0.76)
						lockSubtitle.Size = UDim2.new(0.8, 0, 0, parentSize.Y * 0.18)
...
local function createLockOverlay(parentInstance: Instance)
	...
	titleSizeConstraint.Parent = lockTitle
	...
	subSizeConstraint.MinTextSize = 8
	subSizeConstraint.Parent = lockSubtitle
