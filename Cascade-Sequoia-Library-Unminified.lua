...
--[[
    ...
    @buildDate "2026-07-23T07:11:12.137956Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			local lockIcon = overlay:FindFirstChild("LockIcon")
			local lockText = overlay:FindFirstChild("LockText")

			-- 레이아웃 정렬 최적화 (2개 이상 병합 시 텍스트는 좌측 상단, 자물쇠는 중앙 Row 정중앙 배치)
			if size > 1 then
				...
				if lockText then
					lockText.Visible = isFirst
					if isFirst then
						lockText.AnchorPoint = Vector2.new(0, 0)
						lockText.Position = UDim2.new(0, 12, 0, 10)
						lockText.TextXAlignment = Enum.TextXAlignment.Left
					end
				end
			else
				-- 1개 단독 잠금일 때 (기존 중앙 정렬 유지, Y축은 parentSize.Y 비례 배치)
				if lockIcon then
					lockIcon.Visible = true
					lockIcon.Position = UDim2.new(0.5, 0, 0, parentSize.Y * 0.38)
				end
				if lockText then
					lockText.Visible = true
					lockText.AnchorPoint = Vector2.new(0.5, 0.5)
					lockText.Position = UDim2.new(0.5, 0, 0, parentSize.Y * 0.72)
...
-- 화면 전체에 잠긴 Row가 하나라도 존재하는지 체크하는 헬퍼
local function hasAnyLockedRows()
	...
	for _, desc in ipairs(playerGui:GetDescendants()) do
		if desc.Name == "LockOverlay" and desc:GetAttribute("IsLocked") == true then
			return true
		end
	end
	return false
...
local function createLockOverlay(parentInstance: Instance)
	...
	blocker.BackgroundColor3 = Color3.fromRGB(15, 15, 15) -- 검은색 아크릴 필터로 변경
	...
	blurBg.ImageColor3 = Color3.fromRGB(15, 15, 15) -- 검은색 블러 이미지
	...
	lockIcon.Position = UDim2.new(0.5, 0, 0, parentInstance.AbsoluteSize.Y * 0.38)
	...
	lockText.AnchorPoint = Vector2.new(0.5, 0.5)
	lockText.Position = UDim2.new(0.5, 0, 0, parentInstance.AbsoluteSize.Y * 0.72)
	lockText.Size = UDim2.new(1, 0, 0, 12)
	lockText.BackgroundTransparency = 1
	lockText.Text = "Locked"
	lockText.TextColor3 = Color3.fromRGB(255, 255, 255) -- 하얀색 Locked 텍스트
	lockText.TextSize = 10
	lockText.Font = Enum.Font.SourceSansBold
	lockText.TextTransparency = 1
	lockText.ZIndex = 101
	
	-- 검은색 배경 위에서는 윤곽선(Stroke)이 필요 없으므로 투명하게 설정
	lockText.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
	lockText.TextStrokeTransparency = 1
...
local function wrap(name, make)
	return function(self, ...)
		...
		if typeof(result) == "table" and final ~= nil then
			result.Lock = function(elementSelf, isLocked: boolean)
				...
				if isLocked then
					...
					local lockText = overlay:FindFirstChild("LockText") :: TextLabel

					-- 잠금 상태 전파 및 병합 비주얼 갱신
					updateLockGroups(targetInstance)

					-- Lighting에 BlurEffect 동적 생성 및 트윈 (Size = 56)
					local lighting = game:GetService("Lighting")
					local lockBlur = lighting:FindFirstChild("CascadeLockBlur") :: BlurEffect
					if not lockBlur then
						lockBlur = Instance.new("BlurEffect")
						lockBlur.Name = "CascadeLockBlur"
						lockBlur.Size = 0
						lockBlur.Parent = lighting
					end
					tweenService:Create(lockBlur, tweenInfo, { Size = 56 }):Play()

					-- 페이드인 애니메이션 (검은색 0.15 아크릴 톤 트윈 적용)
					if blocker then
						tweenService:Create(blocker, tweenInfo, { BackgroundTransparency = 0.15 }):Play()
					end
					if blurBg then
						tweenService:Create(blurBg, tweenInfo, { ImageTransparency = 0.4 }):Play()
					end
					if lockIcon and lockIcon.Visible then
						tweenService:Create(lockIcon, tweenInfo, { ImageTransparency = 0.0 }):Play() -- 선명하게 표시
					else
						if lockIcon then lockIcon.ImageTransparency = 1 end
					end
					if lockText and lockText.Visible then
						tweenService:Create(lockText, tweenInfo, { TextTransparency = 0.0 }):Play() -- 선명하게 표시
					else
						if lockText then 
							lockText.TextTransparency = 1 
							lockText.TextStrokeTransparency = 1
					...
					if overlay then
						...
						local t4 = lockText and tweenService:Create(lockText, tweenInfo, { TextTransparency = 1 })
						...
						-- Lighting의 BlurEffect 서서히 제거
						local lighting = game:GetService("Lighting")
						local lockBlur = lighting:FindFirstChild("CascadeLockBlur") :: BlurEffect
						if lockBlur then
							tweenService:Create(lockBlur, tweenInfo, { Size = 0 }):Play()
							task.delay(0.3, function()
								if not hasAnyLockedRows() then
									local currentBlur = lighting:FindFirstChild("CascadeLockBlur")
									if currentBlur then
										currentBlur:Destroy()
									end
								end
