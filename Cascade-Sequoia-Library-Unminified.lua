...
--[[
    ...
    @buildDate "2026-06-21T01:05:37.345112Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local effects = __DIST.i()
	...
	properties.TabAnimation = properties.TabAnimation or "fade"
	...
	local layoutIgnoreFolder = create("Folder")({
		Name = "LayoutIgnore",
		Parent = structures.SidebarList,
	}).__instance

	structures.TabMoveIndicator = create("Frame")({
		Name = "TabMoveIndicator",
		BorderSizePixel = 0,
		Size = UDim2.fromOffset(0, 0),
		AnchorPoint = Vector2.new(0, 0),
		Visible = false,
		ZIndex = 0,
		Parent = layoutIgnoreFolder,
	...
	local bindings = {
		...
		TabAnimation = function(value: "fade" | "move" | "off")
			properties.TabAnimation = value
			if structures.TabMoveIndicator then
				structures.TabMoveIndicator.Visible = (value == "move")
				if value == "move" and object and object.Tabs then
...
local creator = __DIST.d()
	...
	object = binder.Wrap(properties, {
		...
		Selected = function(selected: boolean)
			...
			local tabAnim = window and window.TabAnimation or "fade"

			-- Highlight transparency and text/symbol color
			local targetTransparency
			if tabAnim == "move" then
				targetTransparency = 1
			else
				targetTransparency = selected and self.Theme.Controls.SelectionFocused[2].Value or 1
			end
			...
			if tabAnim == "off" then
				unwrap(structures.Body).BackgroundTransparency = targetTransparency
				unwrap(structures.Title).TextColor3 = targetTitleColor
				unwrap(structures.Title).TextTransparency = targetTitleTransparency
				unwrap(structures.Symbol).ImageColor3 = targetSymbolColor
				unwrap(structures.Symbol).ImageTransparency = targetSymbolTransparency
			else
			...
			if selected then
				...
				if tabAnim == "off" then
					structures.Page.Position = UDim2.new(0, 0, 0, 0)
				else
			...
			-- Handle move indicator (full highlight pill)
			if selected and tabAnim == "move" and window and window.Structures.TabMoveIndicator then
				local indicator = window.Structures.TabMoveIndicator
				local sidebarList = window.Structures.SidebarList
				local tabBody = unwrap(structures.Body)

				local function isClipped(body)
					local parent = body.Parent
					while parent and parent.Name ~= "SidebarList" do
						if parent:IsA("GuiObject") and parent.ClipsDescendants then
							...
							local tabTop = body.AbsolutePosition.Y
							if tabTop >= parentBottom - 2 then
								return true
							end
						end
						parent = parent.Parent
				...
				local lastY, lastX, lastW, lastH, lastClipped = nil, nil, nil, nil, nil
				local function updatePos(instant)
					if not indicator or not tabBody or not sidebarList then return end
					if tabBody.Parent == nil then return end

					local uiPadding = sidebarList:FindFirstChildOfClass("UIPadding")
					local paddingLeft = uiPadding and uiPadding.PaddingLeft.Offset or 0
					local paddingTop = uiPadding and uiPadding.PaddingTop.Offset or 0

					local localY = tabBody.AbsolutePosition.Y - sidebarList.AbsolutePosition.Y + sidebarList.CanvasPosition.Y - paddingTop
					local localX = tabBody.AbsolutePosition.X - sidebarList.AbsolutePosition.X - paddingLeft
					local width = tabBody.AbsoluteSize.X
					local height = tabBody.AbsoluteSize.Y
					local clipped = isClipped(tabBody)
					
					if localY == lastY and localX == lastX and width == lastW and height == lastH and clipped == lastClipped then
						return
					end
					lastY, lastX, lastW, lastH, lastClipped = localY, localX, width, height, clipped

					local targetPos = UDim2.fromOffset(localX, localY)
					local targetSize = UDim2.fromOffset(width, height)

					indicator.Visible = not clipped
				...
				if object._posConnection then
					object._posConnection:Disconnect()
				end
				object._posConnection = tabBody:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
					updatePos(false)
				end)

				if object._sizeConnection then
					object._sizeConnection:Disconnect()
				end
				object._sizeConnection = tabBody:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
					updatePos(false)
				end)

				if object._parentSizeConnection then
					object._parentSizeConnection:Disconnect()
				end
				if tabBody.Parent then
					object._parentSizeConnection = tabBody.Parent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
						updatePos(false)
					end)
				end
			else
				...
				if object._parentSizeConnection then
					object._parentSizeConnection:Disconnect()
					object._parentSizeConnection = nil
				end
			end
		end,
	}, structures.Body)
...
return function(self, properties: any)
	...
	local function openPopover()
		...
		local ok, err = xpcall(function()
			...
			panel.BackgroundColor3 = self.Theme.Controls.MenuButton.MenuBackground[1].Value
			panel.BackgroundTransparency = self.Theme.Controls.MenuButton.MenuBackground[2].Value
			...
			panelStroke.Color = self.Theme.Controls.ViewBorder[1].Value
			panelStroke.Transparency = self.Theme.Controls.ViewBorder[2].Value
			...
			titleLabel.TextColor3 = self.Theme.Text.Primary[1].Value
			titleLabel.TextTransparency = self.Theme.Text.Primary[2].Value
			...
			local function createPureInputField(labelText, defaultValue, layoutOrder, parentFrame)
				...
				label.TextSize = 11
				label.TextColor3 = self.Theme.Text.Secondary[1].Value
				label.TextTransparency = self.Theme.Text.Secondary[2].Value
				...
				input.BackgroundColor3 = self.Theme.Controls.View[1].Value
				input.BackgroundTransparency = self.Theme.Controls.View[2].Value
				input.TextColor3 = self.Theme.Text.Primary[1].Value
				input.TextTransparency = self.Theme.Text.Primary[2].Value
				...
				inputStroke.Color = self.Theme.Controls.ViewBorder[1].Value
				inputStroke.Transparency = self.Theme.Controls.ViewBorder[2].Value
			...
			brightnessLabel.TextColor3 = self.Theme.Text.Secondary[1].Value
			brightnessLabel.TextTransparency = self.Theme.Text.Secondary[2].Value
			...
			opacityLabel.TextColor3 = self.Theme.Text.Secondary[1].Value
			opacityLabel.TextTransparency = self.Theme.Text.Secondary[2].Value
			...
			confirmBtn.BackgroundColor3 = self.Theme.Controls.Selection[1].Value
			confirmBtn.Text = "Confirm"
			confirmBtn.TextColor3 = self.Theme.Text.SelectionPrimary[1].Value
			...
			cancelBtn.BackgroundColor3 = self.Theme.Controls.View[1].Value
			cancelBtn.BackgroundTransparency = self.Theme.Controls.View[2].Value
			cancelBtn.Text = "Cancel"
			cancelBtn.TextColor3 = self.Theme.Text.Primary[1].Value
			cancelBtn.TextTransparency = self.Theme.Text.Primary[2].Value
			...
			cancelStroke.Color = self.Theme.Controls.ViewBorder[1].Value
			cancelStroke.Transparency = self.Theme.Controls.ViewBorder[2].Value
...
local creator = __DIST.d()
	...
	-- 1. Left Chart Area Container
	local chartFrame = create("Frame")({
		Name = "ChartArea",
		Size = UDim2.fromOffset(properties.Size, properties.Size),
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	local totalSum = 0
	for _, item in ipairs(properties.Data) do
	...
	-- 3. If Donut is active, add hollow center mask
	...
	for _, item in ipairs(properties.Data) do
		...
		local legendRow = create("Frame")({
			Name = "LegendRow",
			Size = UDim2.new(0, 200, 0, 18),
			BackgroundTransparency = 1,
			Parent = getRaw(legendStack.Structures.Body),
			...
			create("TextLabel")({
				...
				BackgroundTransparency = 1,
				Text = string.format("%d (%.1f%%)", val, pct),
				TextSize = 12,
...
local creator = __DIST.d()
	...
	-- Initial delay defer trigger to ensure parent bounding size is established
	task.defer(drawChart)

	local object = binder.Wrap(properties, {}, structures.Body or chartContainer)
...
local creator = __DIST.d()
	...
	local function drawChart()
		...
		for i, item in ipairs(data) do
			...
			-- Actual Bar Frame (interactive button)
			local bar = create("ImageButton")({
				Name = "Bar",
				Size = UDim2.new(1, 0, pctHeight, 0),
				Position = UDim2.fromScale(0.5, 1),
				AnchorPoint = Vector2.new(0.5, 1),
