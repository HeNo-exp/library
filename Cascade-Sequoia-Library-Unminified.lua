...
--[[
    ...
    @buildDate "2026-07-23T08:25:32.354439Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, row in ipairs(children) do
		local overlay = row.Parent:FindFirstChild("LockOverlay_" .. row.Name)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			-- 병합 락일 때는 첫 번째 Row의 LockOverlay만 전체 높이로 늘려서 통째로 활성화하고,
			-- 나머지 아래쪽 Row들의 오버레이는 Visible = false 처리하여 팽창 루프 및 비주얼 중복 차단!
			if size > 1 then
				if isFirst then
					overlay.Visible = true
					overlay.Position = UDim2.new(0, -6, 0, row.Position.Y.Offset)
					overlay.Size = UDim2.new(1, 12, 0, totalHeight)

					if blurBg then
						blurBg.Size = UDim2.fromScale(1, 1)
					...
					-- 자물쇠 아이콘은 병합 영역 전체의 정중앙에 배치
					if lockIcon then
						lockIcon.Visible = true
						lockIcon.Position = UDim2.new(0.5, 0, 0, totalHeight / 2)
						tweenService:Create(lockIcon, tweenInfo, { ImageTransparency = 0.0 }):Play()
					end

					-- 제목/소제목은 병합 영역 중 첫 번째 Row의 좌측 상단 영역 내에 정확히 안착
					if lockTitle then
				...
				overlay.Visible = true
				overlay.Position = UDim2.new(0, -6, 0, row.Position.Y.Offset)
				overlay.Size = UDim2.new(1, 12, 0, parentSize.Y)
...
local function createLockOverlay(parentInstance: Instance)
	...
	overlay.Name = "LockOverlay_" .. parentInstance.Name
	
	-- 형제로 배치되므로 고정 사이즈와 고정 포지션을 설정하여 부모 Row의 AutomaticSize 팽창에 간섭을 원천 배제합니다!
	overlay.Size = UDim2.new(1, 12, 0, parentInstance.AbsoluteSize.Y)
	overlay.Position = UDim2.new(0, -6, 0, parentInstance.Position.Y.Offset)
	...
	blocker.Size = UDim2.fromScale(1, 1) -- 오버레이와 100% 동일 가로세로 유지
	...
	blurBg.Size = UDim2.fromScale(1, 1) -- 오버레이와 100% 동일 가로세로 유지
	blurBg.BackgroundTransparency = 1
	blurBg.Image = "rbxassetid://15629910543"
	blurBg.ImageColor3 = Color3.fromRGB(255, 255, 255)
	...
	lockIcon.Position = UDim2.new(0.5, 0, 0, parentInstance.AbsoluteSize.Y * 0.28)
	...
	-- Row의 부모(Form 컨테이너)에 오버레이를 형제(Sibling)로 직접 배치하여 팽창을 원천 방지!
	overlay.Parent = parentInstance.Parent
	return overlay
end
