...
function library:init(title, splash, keybind, deleteprevious)
    ...
    -- Slide up animation and Apple splash screen setup
    local TweenService = game:GetService("TweenService")
    local tweenInfo = TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out)
    
    -- Hide window body off-screen initially to animate it
    if window.Structures and window.Structures.Body then
        ...
        -- Center splash screen if requested
        if splash then
            task.spawn(function()
                -- Find ScreenGui parent of the window
                local screenGui = body.Parent
                if screenGui then
                    local splashFrame = Instance.new("Frame")
                    splashFrame.Name = "AppleSplash"
                    splashFrame.AnchorPoint = Vector2.new(0.5, 0.5)
                    splashFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 25) -- Modern space grey background
                    splashFrame.BackgroundTransparency = 0.15
                    splashFrame.Position = UDim2.new(0.5, 0, 1.5, 0) -- Slide in from bottom
                    splashFrame.Size = UDim2.fromOffset(240, 240)
                    ...
                    local appleIcon = Instance.new("ImageLabel")
                    appleIcon.Name = "AppleIcon"
                    appleIcon.AnchorPoint = Vector2.new(0.5, 0.5)
                    appleIcon.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    appleIcon.BackgroundTransparency = 1
                    appleIcon.Position = UDim2.new(0.5, 0, 0.5, 0)
                    appleIcon.Size = UDim2.fromOffset(110, 110)
                    appleIcon.Image = "rbxassetid://12621719043" -- Original Apple icon
                    appleIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
                    appleIcon.Parent = splashFrame

                    -- Animate Splash In
                    TweenService:Create(splashFrame, tweenInfo, { Position = UDim2.new(0.5, 0, 0.5, 0) }):Play()
                    task.wait(1.8)

                    -- Animate Splash Out
                    local slideOut = TweenService:Create(splashFrame, tweenInfo, { Position = UDim2.new(0.5, 0, 1.5, 0) })
                    slideOut:Play()
            ...
            -- If no splash screen, slide window up immediately
            task.defer(function()
                TweenService:Create(body, tweenInfo, { Position = UDim2.new(0.5, 0, 0.5, 0) }):Play()
            end)
        end
    ...
    if keybind then
        ...
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if input.KeyCode == keybind and not gameProcessed then
                ...
                window.Minimized = not isVisible
            end
        end)
    end
