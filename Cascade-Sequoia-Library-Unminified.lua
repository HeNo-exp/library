...
--[[
    ...
    @buildDate "2026-07-22T15:39:39.677636Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function updateLockGroups(startInstance: Instance)
	...
	for _, group in ipairs(groups) do
		...
		for idx, row in ipairs(group) do
			...
			-- 오버레이 위치/크기 조정 (구분선 덮어쓰기 위해 높이 추가)
			local offsetPosY = 0
			local offsetSizeY = 0
			...
			-- AutomaticSize 버그 방지: AbsoluteSize 기반 Offset 크기 설정
			local parentSize = row.AbsoluteSize
			overlay.Position = UDim2.new(0, 0, 0, offsetPosY)
			overlay.Size = UDim2.new(0, parentSize.X, 0, parentSize.Y + offsetSizeY)
		...
		if typeof(result) == "table" and final ~= nil then
			result.Lock = function(elementSelf, isLocked: boolean)
				...
				if isLocked then
					...
					if blurBg then
						tweenService:Create(blurBg, tweenInfo, { ImageTransparency = 0.7 }):Play()
					end
