...
--[[
    ...
    @buildDate "2026-07-21T16:14:31.851009Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local creator = __DIST.d()
	...
	local function drawChart()
		...
		-- Optional smooth spin-bounce animation if Animated / LiveAnimation is true
		local isAnimated = properties.Animated ~= false and properties.LiveAnimation ~= false
		local rawChart = getRaw(chartFrame)
		if isAnimated then
			rawChart.Rotation = -25
			rawChart.GroupTransparency = 0.3
			tweenService:Create(rawChart, TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
				Rotation = 0,
				GroupTransparency = 0,
			}):Play()
		else
