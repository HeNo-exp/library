...
--[[
    ...
    @buildDate "2026-06-20T03:14:43.953146900+00:00"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local effects = __DIST.i()
	...
	properties.TabTweenInfo = properties.TabTweenInfo or TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	...
	local indicatorFolder = structures.SidebarList:FindFirstChild("IndicatorFolder") :: Folder
	structures.SelectionIndicator = indicatorFolder and indicatorFolder:FindFirstChild("SelectionIndicator") :: Frame
	...
	object.Type = "Window"
	object.TabTweenInfo = properties.TabTweenInfo
	...
	object = binder.Wrap(properties, {
		...
		Selected = function(selected: boolean)
			if selected then
				...
				-- Indicator 이동 로직
				local indicator = window.Structures.SelectionIndicator
				local sidebarList = window.Structures.SidebarList
				if indicator and sidebarList then
					local tabBtn = structures.Body

					local function updateIndicator(useTween)
						if not tabBtn.Parent then return end
						local relativeY = tabBtn.AbsolutePosition.Y - sidebarList.AbsolutePosition.Y + sidebarList.CanvasPosition.Y
						local relativeX = tabBtn.AbsolutePosition.X - sidebarList.AbsolutePosition.X
						local targetPos = UDim2.fromOffset(relativeX, relativeY)
						local targetSize = UDim2.fromOffset(tabBtn.AbsoluteSize.X, tabBtn.AbsoluteSize.Y)

						indicator.Visible = true

						if useTween then
							isSwitching = true
							local tweenInfo = window.TabTweenInfo or TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
							local tween = tweenService:Create(indicator.__instance, tweenInfo, {
								Position = targetPos,
								Size = targetSize,
							})
							tween:Play()
					...
					-- 위치 및 크기 변경 감지 연결
					positionConn = tabBtn:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
						updateIndicator(isSwitching)
					end)
					sizeConn = tabBtn:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
						updateIndicator(isSwitching)
					end)

					-- 초기 위치 적용 (화면에 아직 안 그려졌을 수 있으므로 defer 처리도 병행)
					if tabBtn.AbsoluteSize.Y > 0 and sidebarList.AbsoluteSize.Y > 0 then
						if indicator.Size.X.Offset == 0 and indicator.Size.Y.Offset == 0 then
			...
			structures.Body.Transparency = 1
...
return function(self, properties: any)
	...
	local function openPopover()
		...
		local ok, err = xpcall(function()
			...
			-- 1. Popover Panel Frame 생성 (creator.Create 적용)
			panel = create("Frame")({
			...
			local titleLabel = create("TextLabel")({
				...
				Size = UDim2.new(1, 0, 0, 20),
				BackgroundTransparency = 1,
				Text = properties.Title or "Colorpicker",
				TextSize = 15,
				FontFace = Font.new("rbxassetid://12187365364"),
				LayoutOrder = 1,
				Parent = panel,
			...
			local middleSplit = create("Frame")({
				Name = "MiddleSplit",
			...
			colorWheel = create("ImageButton")({
				Name = "ColorWheel",
				Size = UDim2.fromOffset(120, 120),
				BackgroundTransparency = 1,
				Image = "rbxthumb://type=Asset&id=111925988136154&w=420&h=420",
				ImageTransparency = 0,
				LayoutOrder = 1,
				AutoButtonColor = false,
				Parent = middleSplit,
			...
			selectorDot = create("Frame")({
				...
				Parent = colorWheel,
				__dynamicKeys = {
					BackgroundColor3 = self.Theme.Controls.Toggle.Knob[1],
				},
			...
			local function createPureInputField(labelText, defaultValue, layoutOrder, parentFrame)
				...
				local input = create("TextBox")({
					...
					create("UIStroke")({
						__dynamicKeys = {
							Color = self.Theme.Controls.Slider.ThumbStroke[1],
							Transparency = self.Theme.Controls.Slider.ThumbStroke[2],
						},
					}),
			...
			brightnessTrack = create("ImageButton")({
				Name = "Track",
			...
			brightnessGradient = create("UIGradient")({
			...
			brightnessThumb = create("Frame")({
				Name = "Thumb",
				Size = UDim2.fromOffset(12, 12),
				AnchorPoint = Vector2.new(0.5, 0.5),
				Position = UDim2.new(v_init, 0, 0.5, 0),
				Parent = brightnessTrack,
				__dynamicKeys = {
					BackgroundColor3 = self.Theme.Controls.Slider.Thumb[1],
					BackgroundTransparency = self.Theme.Controls.Slider.Thumb[2],
				},
			...
			local opacityFrame = create("Frame")({
			...
			local opacityTrackBG = create("ImageLabel")({
				Name = "TrackBG",
				Size = UDim2.new(1, -70, 0, 8),
				BackgroundTransparency = 1,
				Image = "rbxassetid://143370504",
			...
			opacityTrack = create("ImageButton")({
				Name = "Track",
				Size = UDim2.fromScale(1, 1),
				BackgroundTransparency = 0,
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
				AutoButtonColor = false,
				Parent = opacityTrackBG,
			...
			opacityGradient = create("UIGradient")({
				Parent = opacityTrack,
			}).__instance
			...
			opacityThumb = create("Frame")({
				Name = "Thumb",
				Size = UDim2.fromOffset(12, 12),
				AnchorPoint = Vector2.new(0.5, 0.5),
				Position = UDim2.new(1 - tempTransparency, 0, 0.5, 0),
				Parent = opacityTrack,
				__dynamicKeys = {
					BackgroundColor3 = self.Theme.Controls.Slider.Thumb[1],
					BackgroundTransparency = self.Theme.Controls.Slider.Thumb[2],
			...
			-- 5. Previews
			local previewsFrame = create("Frame")({
				Name = "PreviewsFrame",
				Size = UDim2.new(1, 0, 0, 24),
				BackgroundTransparency = 1,
				LayoutOrder = 5,
			...
			local newColorBG = create("ImageLabel")({
				Name = "NewColorBG",
			...
			newColorBar = create("Frame")({
				Name = "NewColor",
				Size = UDim2.fromScale(1, 1),
				BackgroundColor3 = tempColor,
				BackgroundTransparency = tempTransparency,
				Parent = newColorBG,
			...
			local originalColorBG = create("ImageLabel")({
				Name = "OriginalColorBG",
				Size = UDim2.new(0.5, -2, 1, 0),
				BackgroundTransparency = 1,
				Image = "rbxassetid://143370504",
				ScaleType = Enum.ScaleType.Tile,
			...
			originalColorBar = create("Frame")({
				Name = "OriginalColor",
				Size = UDim2.fromScale(1, 1),
				BackgroundColor3 = originalColor,
				BackgroundTransparency = originalTransparency,
				Parent = originalColorBG,
