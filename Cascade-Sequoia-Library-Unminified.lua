...
--[[
    ...
    @buildDate "2026-07-22T06:09:32.893306Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local function findRowTitle(body)
	...
	local row = body.Parent
	local searchIndex = row:GetAttribute("SearchIndex")
	if searchIndex and searchIndex ~= "" then
		return searchIndex
	end
	-- Fallback to traversing LeftAccessory
	local left = row:FindFirstChild("LeftAccessory")
	if left then
...
local binder = __DIST.c()
	...
	if properties.Hotkey ~= false and window and window.HotkeyWidget then
		...
		if not hotkeyId then
			hotkeyId = properties.Id or ("hotkey_" .. (string.gsub(string.lower(hotkeyName), "[%s%p]+", "_")))
		end
