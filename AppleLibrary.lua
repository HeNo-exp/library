--[[
    AppleLibrary (Cascade Edition)
    Automatically bundled using custom Python bundler.
    Based on Cascade UI (macOS Sequoia style Roblox UI library).
--]]
...
local function require(name)
    ...
    for alias, replacement in pairs(aliases) do
        if name == alias then
            name = replacement
            break
        elseif name:sub(1, #alias + 1) == alias .. "/" then
            name = replacement .. name:sub(#alias + 1)
            break
        end
    end
    ...
    if not func then
        -- Check with common file suffixes and /init
        if modules[name .. ".luau"] then
            func = modules[name .. ".luau"]
        elseif modules[name .. ".lua"] then
            func = modules[name .. ".lua"]
        elseif modules[name .. "/init.luau"] then
            func = modules[name .. "/init.luau"]
        elseif modules[name .. "/init.lua"] then
            func = modules[name .. "/init.lua"]
        end
    end
    ...
    local res = func()
    module_cache[name] = res
    return res
end

local function make_relative_require(current_module_name)
    return function(name)
        if name:sub(1, 6) == "@self/" then
            name = "./" .. name:sub(7)
        end
        if name:sub(1, 1) == "." or name:sub(1, 2) == ".." then
            -- Resolve relative path
            local parts = {}
            for part in current_module_name:gmatch("[^/]+") do
                table.insert(parts, part)
            end
            table.remove(parts) -- remove file name
...
register("src/init.luau", function()
    local require = make_relative_require("src/init.luau")
--// Imports
...
register("src/types.luau", function()
    local require = make_relative_require("src/types.luau")
-- Base Types
...
export type ComponentContext = ComponentProperties & Components

-- App dumps
export type AppDumpOptions = {
	IncludeInstanceProperties: boolean?,
	IncludeInstanceTree: boolean?,
	Reset: boolean?,
}

export type AppRecorder = {
	Active: boolean,
	App: App,

	Start: (self: AppRecorder, options: AppDumpOptions?) -> AppRecorder,
	Stop: (self: AppRecorder) -> AppRecorder,
	Clear: (self: AppRecorder) -> AppRecorder,
	Dump: (self: AppRecorder, options: AppDumpOptions?) -> string,
}
...
-- Page
export type PageProperties = ScrollingFrame
export type Page = BaseComponent & Components & PageProperties
...
-- DropdownMenu
export type DropdownMenuAnchorConfig = {
	Object: GuiObject?,
	Element: GuiObject?,
	Label: GuiObject?,
	Option: number?,
...
export type DropdownMenuAnchor = GuiObject | DropdownMenuAnchorConfig
...
export type PullDownButton = BaseComponent & Components & PullDownButtonProperties & {
	Option: (Name: string?) -> Frame,
	Remove: (Index: number?) -> nil,
}

-- Notification
export type NotificationProperties = Frame & {
	Title: string,
	Subtitle: string,
	App: string?,
	AppIcon: string?,
...
export type Notification = BaseComponent & Components & NotificationProperties & {
	Close: (self: Notification) -> nil,
}

-- ImageSurface
export type ImageSurfaceProperties = Frame & {
	Image: string?,
	ImageColor: Color3?,
	SurfaceColor: Color3?,
	Gradient: boolean?,
...
export type ImageSurface = BaseComponent & Components & ImageSurfaceProperties

-- Export
export type Components = {
	Window: (self: any, properties: WindowProperties?) -> Window,
	Section: (self: any, properties: SectionProperties?) -> Section,
	Tab: (self: any, properties: TabProperties?) -> Tab,
	Page: (self: any, properties: PageProperties?) -> Page,
	...
	PullDownButton: (self: any, properties: PullDownButtonProperties) -> PullDownButton,
	Notification: (self: any, properties: NotificationProperties) -> Notification,
	ImageSurface: (self: any, properties: ImageSurfaceProperties) -> ImageSurface,
}
...
register("src/components/Button.luau", function()
    local require = make_relative_require("src/components/Button.luau")
local types = require("@types")
...
register("src/components/Form.luau", function()
    local require = make_relative_require("src/components/Form.luau")
local types = require("@types")
...
register("src/components/HStack.luau", function()
    local require = make_relative_require("src/components/HStack.luau")
local types = require("@types")
...
register("src/components/init.luau", function()
    local require = make_relative_require("src/components/init.luau")
--// Imports
local binder = require("@modules/binder")
local appRecorder = require("@modules/appRecorder")
...
register("src/components/KeybindField.luau", function()
    local require = make_relative_require("src/components/KeybindField.luau")
local types = require("@types")
...
register("src/components/Label.luau", function()
    local require = make_relative_require("src/components/Label.luau")
local types = require("@types")
...
register("src/components/Page.luau", function()
    local require = make_relative_require("src/components/Page.luau")
local types = require("@types")
...
register("src/components/PageSection.luau", function()
    local require = make_relative_require("src/components/PageSection.luau")
local types = require("@types")
...
register("src/components/PopUpButton.luau", function()
    local require = make_relative_require("src/components/PopUpButton.luau")
local types = require("@types")
...
register("src/components/PullDownButton.luau", function()
    local require = make_relative_require("src/components/PullDownButton.luau")
local types = require("@types")
...
register("src/components/RadioButtonGroup.luau", function()
    local require = make_relative_require("src/components/RadioButtonGroup.luau")
local types = require("@types")
...
register("src/components/Row.luau", function()
    local require = make_relative_require("src/components/Row.luau")
local types = require("@types")
...
register("src/components/Section.luau", function()
    local require = make_relative_require("src/components/Section.luau")
local types = require("@types")
...
register("src/components/Slider.luau", function()
    local require = make_relative_require("src/components/Slider.luau")
local types = require("@types")
...
register("src/components/Stepper.luau", function()
    local require = make_relative_require("src/components/Stepper.luau")
local types = require("@types")
...
register("src/components/Symbol.luau", function()
    local require = make_relative_require("src/components/Symbol.luau")
local types = require("@types")
...
register("src/components/Tab.luau", function()
    local require = make_relative_require("src/components/Tab.luau")
local types = require("@types")
	...
	object = binder.Wrap(properties, {
		...
		Selected = function(selected: boolean)
			if selected then
				...
				-- Sliding selection pill animation
				if window and window.Structures and window.Structures.SidebarList then
					local selectionPillFolder = window.Structures.SidebarList:FindFirstChild("SelectionPillFolder")
					local selectionPill = selectionPillFolder and selectionPillFolder:FindFirstChild("SelectionPill")
					if selectionPill then
						task.defer(function()
							if not structures.Body.__instance.Parent or not window.Structures.SidebarList.Parent then return end
							local relativePos = structures.Body.__instance.AbsolutePosition - window.Structures.SidebarList.AbsolutePosition
							local canvasY = window.Structures.SidebarList.CanvasPosition.Y
							local targetPos = UDim2.fromOffset(relativePos.X, relativePos.Y + canvasY)
							local targetSize = UDim2.fromOffset(structures.Body.__instance.AbsoluteSize.X, structures.Body.__instance.AbsoluteSize.Y)
							
							if not selectionPill.Visible then
								selectionPill.Position = targetPos
								selectionPill.Size = targetSize
								selectionPill.Visible = true
							else
								local tweenService = game:GetService("TweenService")
								local tweenInfo = TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
								tweenService:Create(selectionPill, tweenInfo, {
									Position = targetPos,
									Size = targetSize
								}):Play()
							end
			...
			local targetTextColor = selected and self.Theme.Text.SelectionPrimary[1].Value or self.Theme.Text.Primary[1].Value
			local targetTextTransparency = selected and self.Theme.Text.SelectionPrimary[2].Value or self.Theme.Text.Primary[2].Value
			...
			tweenService:Create(structures.Title, tweenInfo, { TextColor3 = targetTextColor, TextTransparency = targetTextTransparency }):Play()
			tweenService:Create(structures.Symbol, tweenInfo, { ImageColor3 = targetSymbolColor, ImageTransparency = targetSymbolTransparency }):Play()

			structures.Page.Parent = selected and window and window.__container or nil
		end,
	}, structures.Body)

	object.Navigate = function(self, targetPage: types.Page)
...
register("src/components/TextField.luau", function()
    local require = make_relative_require("src/components/TextField.luau")
local types = require("@types")
...
register("src/components/TitleStack.luau", function()
    local require = make_relative_require("src/components/TitleStack.luau")
local types = require("@types")
...
register("src/components/Toggle.luau", function()
    local require = make_relative_require("src/components/Toggle.luau")
local types = require("@types")
...
register("src/components/VStack.luau", function()
    local require = make_relative_require("src/components/VStack.luau")
local types = require("@types")
...
register("src/components/Window.luau", function()
    local require = make_relative_require("src/components/Window.luau")
-- We uh
...
return function(self, properties: types.WindowProperties): types.Window
	...
	local windowControls, windowControlsCKeys = require("@structures/WindowControls")(self, properties)
	...
	structures.Body = binder.Apply(
		...
		create("Frame")({
			...
			create("Frame")({
				...
				BorderColor3 = Color3.fromRGB(0, 0, 0),
				BorderSizePixel = 0,
				Size = properties.Sidebar ~= false and UDim2.new(0, 200, 1, 0) or UDim2.new(0, 0, 1, 0),
				ClipsDescendants = true,
			...
			create("Frame")({
				...
				Visible = properties.Sidebar ~= false,
				__dynamicKeys = {
			...
			create("Frame")({
				...
				LayoutOrder = 2,
				Position = UDim2.fromScale(1, 0),
				Size = properties.Sidebar ~= false and UDim2.new(1, -201, 1, 0) or UDim2.new(1, 0, 1, 0),
	...
	do -- Sidebar button
		...
		structures.Titlebar.SidebarButton.Visible = properties.Sidebar ~= false
		...
		userInputService.InputChanged:Connect(function(input)
			...
			then
				if dragging then
					...
					local currentPos = structures.Body.__instance.Position
					local tilt = math.clamp((newPosition.X.Offset - currentPos.X.Offset) * 0.1, -4, 4)
...
register("src/components/ImageSurface/init.luau", function()
    local require = make_relative_require("src/components/ImageSurface/init.luau")
local types = require("@types")
...
register("src/components/ImageSurface/ui.luau", function()
    local require = make_relative_require("src/components/ImageSurface/ui.luau")
...
register("src/components/Notification/init.luau", function()
    local require = make_relative_require("src/components/Notification/init.luau")
local types = require("@types")
...
register("src/components/Notification/ui.luau", function()
    local require = make_relative_require("src/components/Notification/ui.luau")
local types = require("@types")

return function(self: any, properties: types.NotificationProperties, state: { Age: number }, MAX_NOTIFICATIONS: number)
	local creator = require("@modules/creator")
...
register("src/components/RadioCardGroup/init.luau", function()
    local require = make_relative_require("src/components/RadioCardGroup/init.luau")

end)

register("src/components/RadioCardGroup/ui.luau", function()
    local require = make_relative_require("src/components/RadioCardGroup/ui.luau")

end)

register("src/modules/appRecorder.luau", function()
    local require = make_relative_require("src/modules/appRecorder.luau")
--// Imports
local services = require("@modules/services")
...
register("src/modules/binder.luau", function()
    local require = make_relative_require("src/modules/binder.luau")
local binder = {}
...
register("src/modules/canvasFader.luau", function()
    local require = make_relative_require("src/modules/canvasFader.luau")
local services = require("@modules/services")
local tweenService = services.TweenService
...
register("src/modules/creator.luau", function()
    local require = make_relative_require("src/modules/creator.luau")
local types = require("@types")
...
register("src/modules/dropdownMenu.luau", function()
    local require = make_relative_require("src/modules/dropdownMenu.luau")
--// Imports
...
register("src/modules/notificationList.luau", function()
    local require = make_relative_require("src/modules/notificationList.luau")
...
register("src/modules/resizeHandler.luau", function()
    local require = make_relative_require("src/modules/resizeHandler.luau")
...
register("src/modules/services.luau", function()
    local require = make_relative_require("src/modules/services.luau")
--// Imports
local utility = require("@modules/utility")
...
register("src/modules/symbols.luau", function()
    local require = make_relative_require("src/modules/symbols.luau")
return {
...
register("src/modules/utility.luau", function()
    local require = make_relative_require("src/modules/utility.luau")
--// Types
type ExpectedObject = ((...any) -> ...any) | Instance | ServiceProvider
...
register("src/modules/effects/blur.luau", function()
    local require = make_relative_require("src/modules/effects/blur.luau")
--[[
...
register("src/modules/effects/init.luau", function()
    local require = make_relative_require("src/modules/effects/init.luau")
return {
	UIBlur = require("@effects/blur"),
}

end)

register("src/structures/DropdownMenu.luau", function()
    local require = make_relative_require("src/structures/DropdownMenu.luau")
--// Imports
local creator = require("@modules/creator")

--// Types
type DropdownMenuStructureConfig = {
	Name: string?,
}

return function(self, config: DropdownMenuStructureConfig?)
	--// Variables
	local create = creator.Create
...
register("src/structures/TextField.luau", function()
    local require = make_relative_require("src/structures/TextField.luau")
return function(self)
	--// Imports
	local creator = require("@modules/creator")
	local services = require("@modules/services")
...
register("src/structures/Titlebar.luau", function()
    local require = make_relative_require("src/structures/Titlebar.luau")
return function(self)
	--// Imports
	local creator = require("@modules/creator")
...
register("src/structures/TitleStack.luau", function()
    local require = make_relative_require("src/structures/TitleStack.luau")
return function(self)
	--// Imports
	local creator = require("@modules/creator")
...
register("src/structures/WindowControls.luau", function()
    local require = make_relative_require("src/structures/WindowControls.luau")
...
register("src/themes/accents.luau", function()
    local require = make_relative_require("src/themes/accents.luau")
--// Private Methods
...
register("src/themes/Dark.luau", function()
    local require = make_relative_require("src/themes/Dark.luau")
--// Imports
local creator = require("@modules/creator")
...
register("src/themes/init.luau", function()
    local require = make_relative_require("src/themes/init.luau")
return {
...
register("src/themes/Light.luau", function()
    local require = make_relative_require("src/themes/Light.luau")
--// Imports
local creator = require("@modules/creator")
...
register("packages/fusion.lua", function()
    local require = make_relative_require("packages/fusion.lua")
return require("./_index/elttob_fusion@0.3.0/fusion/src")

end)

register("packages/_index/elttob_fusion@0.3.0/fusion/src/External.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/External.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/ExternalDebug.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/ExternalDebug.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/init.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/init.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/RobloxExternal.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/RobloxExternal.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Types.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Types.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/ExternalTime.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/ExternalTime.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/getTweenDuration.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/getTweenDuration.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/getTweenRatio.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/getTweenRatio.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/lerpType.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/lerpType.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/packType.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/packType.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/Spring.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/Spring.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
-- Animation
local ExternalTime = require("../Animation/ExternalTime")
local Stopwatch = require("../Animation/Stopwatch")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/springCoefficients.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/springCoefficients.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/Stopwatch.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/Stopwatch.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/Tween.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/Tween.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
local getTweenDuration = require("../Animation/getTweenDuration")
-- Utility
local nicknames = require("../Utility/nicknames")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/unpackType.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Animation/unpackType.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Colour/Oklab.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Colour/Oklab.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Colour/sRGB.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Colour/sRGB.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/castToGraph.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/castToGraph.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/change.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/change.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/depend.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/depend.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/evaluate.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/evaluate.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/Observer.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Graph/Observer.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/applyInstanceProps.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/applyInstanceProps.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Attribute.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Attribute.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/AttributeChange.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/AttributeChange.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/AttributeOut.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/AttributeOut.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Child.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Child.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Children.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Children.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/defaultProps.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/defaultProps.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Hydrate.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Hydrate.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/New.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/New.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/OnChange.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/OnChange.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/OnEvent.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/OnEvent.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Out.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Instances/Out.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Logging/formatError.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Logging/formatError.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Logging/messages.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Logging/messages.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Logging/parseError.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Logging/parseError.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/checkLifetime.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/checkLifetime.luau")
...
local Types = require("../Types")
local External = require("../External")
local whichLivesLonger = require("../Memory/whichLivesLonger")
local nameOf = require("../Utility/nameOf")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/deriveScope.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/deriveScope.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
local Types = require("../Types")
local ExternalDebug = require("../ExternalDebug")
local deriveScopeImpl = require("../Memory/deriveScopeImpl")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/deriveScopeImpl.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/deriveScopeImpl.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
local scopePool = require("../Memory/scopePool")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/doCleanup.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/doCleanup.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
local External = require("../External")
local scopePool = require("../Memory/scopePool")
local poisonScope = require("../Memory/poisonScope")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/innerScope.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/innerScope.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/legacyCleanup.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/legacyCleanup.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/needsDestruction.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/needsDestruction.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/poisonScope.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/poisonScope.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/scoped.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/scoped.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/scopePool.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/scopePool.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/whichLivesLonger.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Memory/whichLivesLonger.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/castToState.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/castToState.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/Computed.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/Computed.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
local never = require("../Utility/never")
-- Graph
local depend = require("../Graph/depend")
-- State
local castToState = require("../State/castToState")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/ForKeys.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/ForKeys.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
local doCleanup = require("../Memory/doCleanup")
-- State
local For = require("../State/For")
local Value = require("../State/Value")
local Computed = require("../State/Computed")
local ForTypes = require("../State/For/ForTypes")
-- Logging
local parseError = require("../Logging/parseError")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/ForPairs.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/ForPairs.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/ForValues.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/ForValues.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/peek.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/peek.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/updateAll.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/updateAll.luau")
return nil -- dummy file so I can write tests
end)

register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/Value.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/Value.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/For/Disassembly.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/For/Disassembly.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/For/ForTypes.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/For/ForTypes.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/State/For/init.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/State/For/init.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
local Disassembly = require("../../State/For/Disassembly")
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/Contextual.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/Contextual.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/isSimilar.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/isSimilar.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/merge.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/merge.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/nameOf.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/nameOf.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/never.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/never.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/nicknames.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/nicknames.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/Safe.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/Safe.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
register("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/xtypeof.luau", function()
    local require = make_relative_require("packages/_index/elttob_fusion@0.3.0/fusion/src/Utility/xtypeof.luau")
--!strict
--!nolint LocalUnused
--!nolint LocalShadow
local task = nil -- Disable usage of Roblox's task scheduler
...
--// AppleLibrary Compatibility Layer
local cascade = require("@src/init")

local library = {}

function library:init(title, subtitle, keybind, deleteprevious)
    if deleteprevious then
        local CoreGui = game:GetService("CoreGui")
        local Players = game:GetService("Players")
        local localPlayer = Players.LocalPlayer
        local parent = localPlayer and localPlayer:FindFirstChildOfClass("PlayerGui") or CoreGui
        
        for _, child in ipairs(parent:GetChildren()) do
            if child:IsA("ScreenGui") and (child.Name == "Cascade" or child.Name == "AppleLibrary") then
                child:Destroy()
            end
        end
    end
    ...
    if type(displaySubtitle) == "boolean" then
        displaySubtitle = ""
    end

    -- Create ScreenGui wrapper to handle core UI setup
    local app = cascade.New({
        WindowPill = true,
        Theme = cascade.Themes.Dark, -- AppleLibrary dark mode by default
        Accent = cascade.Accents.Blue,
    })

    -- Create Cascade Window
    local window = app:Window({
        Title = title or "Apple Library",
        Subtitle = displaySubtitle or "",
        Searching = false,
        Draggable = true,
        Resizable = false,
        Dropshadow = true,
    ...
    -- Slide up animation (No Apple splash screen)
    local TweenService = game:GetService("TweenService")
    local tweenInfo = TweenInfo.new(0.8, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out)
    
    if window.Structures and window.Structures.Body then
        local body = window.Structures.Body.__instance or window.Structures.Body
        body.Position = UDim2.new(0.5, 0, 1.5, 0) -- Start off-screen
        task.defer(function()
            TweenService:Create(body, tweenInfo, { Position = UDim2.new(0.5, 0, 0.5, 0) }):Play()
        end)
    end

    -- Handle Keybind for visibility toggling
    local compatWindow = {}
    local currentKeybind = keybind or Enum.KeyCode.RightShift
    
    local UserInputService = game:GetService("UserInputService")
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if input.KeyCode == currentKeybind and not gameProcessed then
            compatWindow:ToggleVisible()
        end
    end)

    compatWindow._app = app
    compatWindow._window = window
    compatWindow._tabs = {}
    compatWindow._currentTab = nil
    ...
    local sidebar = window.Structures.Sidebar
    if sidebar then
        -- Shrink SidebarList slightly to make space for profile
        if window.Structures.SidebarList then
            window.Structures.SidebarList.Size = UDim2.new(1, 0, 1, -142)
        end

        local profileCard = Instance.new("Frame")
        profileCard.Name = "ProfileCard"
        profileCard.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
        profileCard.BackgroundTransparency = 0.5
        profileCard.Position = UDim2.new(0, 10, 1, -80)
        ...
        -- Info label (Health & Uptime)
        local infoLabel = Instance.new("TextLabel")
        infoLabel.Name = "InfoLabel"
        infoLabel.BackgroundTransparency = 1
        infoLabel.Position = UDim2.new(0, 65, 0, 26)
        infoLabel.Size = UDim2.new(1, -95, 0, 36)
        ...
        task.spawn(function()
            ...
            while task.wait(1) do
                ...
                infoLabel.Text = string.format("HP: <font color='#ff5e56'>%d</font>\nUptime: <font color='#27c83f'>%dm %ds</font>", hp, minutes, seconds)
            end
        end)

        -- Settings button click behavior
        settingsBtn.MouseButton1Click:Connect(function()
            -- Open keybind selection window
            local settingsWin = app:Window({
                Title = "UI Settings",
                Subtitle = "Configure UI options",
            ...
            settingsWin.Structures.Body.Position = UDim2.fromScale(0.5, 0.5)

            local sSec = settingsWin:Section({ Title = "Settings", Disclosure = false })
            local sTab = sSec:Tab({ Title = "Controls", Selected = true })
            local sForm = sTab:Form()

            local kRow = sForm:Row()
            kRow:Left():Label({ Text = "Toggle Keybind", TextXAlignment = Enum.TextXAlignment.Left })
            kRow:Right():KeybindField({
                Value = currentKeybind,
                ValueChanged = function(self, newKey)
                    currentKeybind = newKey
                    print("Toggle key changed to:", newKey)
                end
            })
        end)
    end
    ...
    local function getActiveTab()
        if not compatWindow._currentTab then
            compatWindow:Section("General")
        end
        return compatWindow._currentTab
    end

    function compatWindow:ToggleVisible()
        local screenGui = window.Structures and window.Structures.Body and window.Structures.Body.Parent and window.Structures.Body.Parent.Parent
    ...
    function compatWindow:GreenButton(callback)
        local zoomButton = window.Structures.WindowControls.Zoom
        if zoomButton then
    ...
    function compatWindow:TempNotify(text1, text2, icon)
        app:Notification({
            Title = text1,
            Subtitle = text2,
            App = "AppleLibrary",
            Duration = 5,
            AppIcon = icon or "rbxassetid://12608259004",
        })
    end

    function compatWindow:Notify(txt1, txt2, b1, icohn, callback)
        local popup = app:Window({
            Title = txt1,
            Subtitle = txt2,
        ...
        popup.Structures.Body.Position = UDim2.fromScale(0.5, 0.5)

        local section = popup:Section({ Title = "Notification", Disclosure = false })
        local tab = section:Tab({ Title = "Details", Selected = true })
        local form = tab:Form()
        local row = form:Row()
        ...
        row:Right():Button({
            ...
            Pushed = function()
                ...
                task.delay(0.25, popup.Structures.Body.Destroy)
                if callback then callback() end
            end
        })
    end

    function compatWindow:Notify2(txt1, txt2, b1, b2, icohn, callback, callback2)
        local popup = app:Window({
            Title = txt1,
            Subtitle = txt2,
    ...
    function compatWindow:Divider(name)
        window:Section({ Title = name, Disclosure = false })
    end

    function compatWindow:Section(name)
        local tab = window:Tab({
            Title = name,
            Selected = (#window.Tabs == 0),
        })

        local compatSection = {}
        compatSection._tab = tab
        compatSection._window = window
        compatSection._currentForm = nil

        function compatSection:_getForm()
            if not self._currentForm then
                self._currentForm = self._tab:Form()
            end
            return self._currentForm
        end

        function compatSection:Divider(dividerName)
            local pageSection = self._tab:PageSection({ Title = dividerName })
            self._currentForm = pageSection:Form()
        end

        function compatSection:Button(buttonName, callback, buttonText)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = buttonName })
            row:Left():Label({ Text = buttonName, TextXAlignment = Enum.TextXAlignment.Left })
            row:Right():Button({
                Label = buttonText or "Click",
                State = "Primary",
                Pushed = function()
                    if callback then callback() end
                end
        ...
        function compatSection:Label(labelName)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = labelName })
            row:Left():Label({ Text = labelName, TextXAlignment = Enum.TextXAlignment.Left })
        end

        function compatSection:Switch(switchName, defaultmode, callback)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = switchName })
            row:Left():Label({ Text = switchName, TextXAlignment = Enum.TextXAlignment.Left })
            row:Right():Toggle({
                Value = defaultmode == true,
        ...
        function compatSection:Toggle(toggleName, defaultmode, callback)
            return self:Switch(toggleName, defaultmode, callback)
        end

        function compatSection:TextField(fieldName, placeholder, callback)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = fieldName })
            row:Left():Label({ Text = fieldName, TextXAlignment = Enum.TextXAlignment.Left })
            row:Right():TextField({
        ...
        -- Aliases
        function compatSection:Input(fieldName, placeholder, callback)
            return self:TextField(fieldName, placeholder, callback)
        end

        function compatSection:Slider(sliderName, min, max, default, callback)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = sliderName })
            row:Left():Label({ Text = sliderName, TextXAlignment = Enum.TextXAlignment.Left })
            local valLabel = row:Left():Label({ Text = tostring(default or min or 0), TextXAlignment = Enum.TextXAlignment.Left })
        ...
        function compatSection:Keybind(bindName, default, callback)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = bindName })
            row:Left():Label({ Text = bindName, TextXAlignment = Enum.TextXAlignment.Left })
            local bind = row:Right():KeybindField({
        ...
        function compatSection:Colorpicker(cpName, default, callback)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = cpName })
            row:Left():Label({ Text = cpName, TextXAlignment = Enum.TextXAlignment.Left })
            
            local currentColor = default or Color3.fromRGB(255, 255, 255)
            
            local triggerBtn = row:Right():Button({
                Label = "Pick Color",
                State = "Secondary",
            })

            triggerBtn.Structures.Body.MouseButton1Click:Connect(function()
                -- Resolve picker window using the cached window object
                local picker = self._window:Window({
                    Title = "Color Picker: " .. cpName,
                    Size = UDim2.fromOffset(300, 160),
                    Resizable = false,
                    Draggable = true,
                    CanExit = true,
                ...
                picker.Structures.Body.Position = UDim2.fromScale(0.5, 0.5)

                local pPage = picker:Page()
                pPage.Structures.Page.Parent = picker.__container
                local pForm = pPage:Form()

                local rRow = pForm:Row()
                rRow:Left():Label({ Text = "R", TextXAlignment = Enum.TextXAlignment.Left })
                local rValLabel = rRow:Left():Label({ Text = tostring(math.round(currentColor.R * 255)), TextXAlignment = Enum.TextXAlignment.Left })
                local rSlider = rRow:Right():Slider({
                    Minimum = 0,
                    Maximum = 255,
                    Value = currentColor.R * 255,
                })

                local gRow = pForm:Row()
                gRow:Left():Label({ Text = "G", TextXAlignment = Enum.TextXAlignment.Left })
                local gValLabel = gRow:Left():Label({ Text = tostring(math.round(currentColor.G * 255)), TextXAlignment = Enum.TextXAlignment.Left })
                local gSlider = gRow:Right():Slider({
                    Minimum = 0,
                    Maximum = 255,
                    Value = currentColor.G * 255,
                })

                local bRow = pForm:Row()
                bRow:Left():Label({ Text = "B", TextXAlignment = Enum.TextXAlignment.Left })
                local bValLabel = bRow:Left():Label({ Text = tostring(math.round(currentColor.B * 255)), TextXAlignment = Enum.TextXAlignment.Left })
                local bSlider = bRow:Right():Slider({
                    Minimum = 0,
                    Maximum = 255,
                ...
                local function updateColor()
                    local r = rSlider.Value / 255
                    local g = gSlider.Value / 255
                    local b = bSlider.Value / 255
                    currentColor = Color3.new(r, g, b)
                    
                    rValLabel.Text = tostring(math.round(rSlider.Value))
                    gValLabel.Text = tostring(math.round(gSlider.Value))
                    bValLabel.Text = tostring(math.round(bSlider.Value))
                    
                    if callback then callback(currentColor) end
                end

                rSlider.Structures.Dragger.DragContinue:Connect(updateColor)
                gSlider.Structures.Dragger.DragContinue:Connect(updateColor)
                bSlider.Structures.Dragger.DragContinue:Connect(updateColor)
            end)
        end

        function compatSection:Dropdown(ddName, list, default, callback, max)
            -- Handle optional default value or callback placement
            ...
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = ddName })
            row:Left():Label({ Text = ddName, TextXAlignment = Enum.TextXAlignment.Left })
            ...
            if type(default) == "table" then
                ...
                for _, val in ipairs(default) do
                    if type(val) == "string" then
                        ...
                        if idx then table.insert(defaultIndex, idx) end
                    elseif type(val) == "number" then
                        table.insert(defaultIndex, val)
                    end
                end
            elseif type(default) == "string" then
                local idx = table.find(list, default)
                if idx then defaultIndex = idx end
                ...
                ValueChanged = function(self, value)
                    if type(value) == "table" then
                        ...
                        for _, idx in ipairs(value) do
                            table.insert(selectedStrs, self.Options[idx])
                        end
                        if callback then callback(selectedStrs) end
                    else
                        local selectedStr = self.Options[value]
                        if callback then callback(selectedStr) end
                    end
                end
            })
            return popUp
        ...
        function compatSection:Stepper(stepperName, min, max, default, step, callback)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = stepperName })
            row:Left():Label({ Text = stepperName, TextXAlignment = Enum.TextXAlignment.Left })
            
            local stepVal = row:Right():Stepper({
                Minimum = min or 0,
                Maximum = max or 100,
                Value = default or min or 0,
                Step = step or 1,
        ...
        function compatSection:Radio(radioName, options, default, callback)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = radioName })
            row:Left():Label({ Text = radioName, TextXAlignment = Enum.TextXAlignment.Left })
            
            local defaultIndex = 1
            if type(default) == "string" then
                local idx = table.find(options, default)
                if idx then defaultIndex = idx end
            elseif type(default) == "number" then
                defaultIndex = default
            end

            local radioGroup = row:Right():RadioButtonGroup({
                Options = options or {},
                Value = defaultIndex,
                ValueChanged = function(self, value)
                    local selectedStr = self.Options[value]
        ...
        function compatSection:Header(headerName)
            local form = self:_getForm()
            local row = form:Row()
            row:Left():Label({ Text = "<b>" .. headerName .. "</b>", TextXAlignment = Enum.TextXAlignment.Left })
        end

        function compatSection:Paragraph(pTitle, pDesc)
            local form = self:_getForm()
            local row = form:Row({ SearchIndex = pTitle })
            row:Left():TitleStack({
                Title = pTitle,
                Subtitle = pDesc or "",
            })
        ...
        function compatSection:SubLabel(subLabelName)
            local form = self:_getForm()
            local row = form:Row()
            row:Left():Label({ Text = "<font size=\"12\" color=\"#8e8e93\">" .. subLabelName .. "</font>", TextXAlignment = Enum.TextXAlignment.Left })
        end

        function compatSection:Spacer(size)
            local form = self:_getForm()
            local height = size or 10
            ...
            spacerFrame.Parent = form.__container
        end

        function compatSection:Select()
            self._tab.Selected = true
        end

        compatWindow._currentTab = tab
        table.insert(compatWindow._tabs, compatSection)

        return compatSection
    end
