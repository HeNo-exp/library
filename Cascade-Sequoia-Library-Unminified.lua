...
--[[
    ...
    @buildDate "2026-06-20T12:54:14.280511Z"
    @buildCfg "Task"
    @buildVers "0.0.0"
...
local creator = __DIST.d()
	...
	local function updateSizing()
		...
		local radius = math.min(absSize.X, absSize.Y) / 2
...
local creator = __DIST.d()
	...
	local function updatePlot()
		...
		local plotW = absSize.X - (paddingX * 2)
		local plotH = absSize.Y - paddingYTop - paddingYBottom
		...
		for i, label in ipairs(xAxisLabels) do
			...
			if pt and label then
				label.Position = UDim2.fromOffset(pt.X, absSize.Y - 12)
			end
		end
	end
